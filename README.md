...
- **상태 대시보드** — 두 서버의 실행/정지 상태, 내부 포트, 메모리(RSS), 시스템 RAM/Load/Uptime
- **전원 제어** — 서버별 시작 / 정지 / 재시작, 전체 시작·정지(`~/start.sh`·`~/stop.sh`)
- **실시간 콘솔** — `tmux capture-pane`로 콘솔 출력 표시 + 명령 입력(`list`, `say`, `op` 등)
- **로그인 보호** — 단일 비밀번호 + 세션 쿠키, 기본 `127.0.0.1` 바인딩(외부 미노출)
